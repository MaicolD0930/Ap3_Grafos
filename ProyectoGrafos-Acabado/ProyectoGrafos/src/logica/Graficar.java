...
public class Graficar extends JFrame {
    ...
    private char[] nodos;
    private LGrafo grafo;

    public Graficar(LGrafo grafo) {
        this.grafo = grafo;
        this.matrizAdyacencia = grafo.MatrizAdyacenciaint();
        this.nodos = grafo.Idchar();
        ...
        for (int i = 0; i < numVertices; i++) {
            for (int j = 0; j < numVertices; j++) {
                if (matrizAdyacencia[i][j] != 0) {
                    ...
                    String peso = Integer.toString(grafo.DistanciaCamino(nodos[i]+"", nodos[j]+""));
